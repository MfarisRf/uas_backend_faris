...
class User63Controller extends Controller
{
    public function dashboardPage63()
    {
        ...
        $getUser = $client->request('GET', "{$API_URL}/user63")->getBody()->getContents();
    ...
    public function profilePage63()
    {
        ...
        $getUser = $client->request('GET', "{$API_URL}/user63/{$user->id}")->getBody()->getContents();
        $getAgama = $client->request('GET', "{$API_URL}/agama63")->getBody()->getContents();
    ...
    public function detailPage63(Request $request, $id)
    {
        $API_URL = env('API_URL', "http://localhost/BackEnd/UAS-Backend/public/api");

        $client = new Client();
        $getUser = $client->request('GET', "{$API_URL}/user63/{$id}")->getBody()->getContents();
        $user = json_decode($getUser, true)['data'];
    ...
    public function putUserDetail63(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'name' => 'required',
            'email' => 'required|email|unique:users63,email,' . $user->id,
            'alamat' => 'required',
        ...
        $putUser = $client->request('PUT', "{$API_URL}/user63/{$user->id}", [
            'json' => [
                'name' => $request->name,
                'email' => $request->email,
    ...
    public function putUserAgama63(Request $request, $id)
    {
        ...
        $putUserAgama = $client->request('PUT', "{$API_URL}/user63/{$id}/agama", [
            'json' => [
    ...
    public function putUserStatus63(Request $request, $id)
    {
        ...
        $putUserStatus = $client->request('PUT', "{$API_URL}/user63/{$id}/status", [
            'json' => [
    ...
    public function putUserPassword63(Request $request)
    {
        ...
        $putUserPassword = $client->request('PUT', "{$API_URL}/user63/{$user->id}/password", [
            'json' => [
    ...
    public function putUserPhoto63(Request $request)
    {
        ...
        $putUserPhoto = $client->request('PUT', "{$API_URL}/user63/{$id}/photo", [
            'headers' => [
    ...
    public function putUserPhotoKTP63(Request $request)
    {
        $API_URL = env('API_URL', "http://localhost/BackEnd/UAS-Backend/public/api");
        ...
        $putUserPhoto = $client->request('PUT', "{$API_URL}/user63/{$id}/photoKTP", [
            'headers' => [
                'Accept' => 'application/json',
            ],
            'json' => [
    ...
    public function deleteUser63($id)
    {
        ...
        $deleteUser = $client->request('DELETE', "{$API_URL}/user63/{$id}", [
            'headers' => [
                'Accept' => 'application/json',
            ],
        ])->getBody()->getContents();
